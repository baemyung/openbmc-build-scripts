...
/home/myungbae/Codes/cppchk-ghe-1120/openbmc-build-scripts/bmc-codescan-prep.sh fw1120.00-1.67 


EOF_SCRIPT

#chmod a+x "${WORKSPACE}/build-codescan.sh"

# Give the Docker image a name based on the distro,tag,arch,and target
img_name=${img_name:-openbmc/${distro}:${img_tag}-${target}-codescan}
...
docker run \
    ...
    ${mount_workspace_dir} \
    "${img_name}" \
    "${WORKSPACE}/openbmc-build-scripts/bmc-codescan-prep.sh"
