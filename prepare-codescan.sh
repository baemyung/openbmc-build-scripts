...
if [ "${INTERACTIVE}" ]; then
    ...
    UNIT_TEST="${UNIT_TEST_SCRIPT_DIR}/bmc-codescan-prep.sh fw1120.00-1.67 "
fi
...
# meant to be split
echo "RUN.... WORKSPACE=($WORKSPACE) DOCKER_WORKDIR=($DOCKER_WORKDIR)"
...
mkdir -p ${CODESCANDIR}
CODESCANDIR=$(cd $CODESCANDIR; pwd)


## DIRECTORIES (full absolute path)
mkdir -p ${CODESCANDIR}/repos/${BRANCHSHORT}
export REPODIR=$(cd ${CODESCANDIR}/repos/${BRANCHSHORT}; pwd)
