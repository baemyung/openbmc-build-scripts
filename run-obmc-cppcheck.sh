...
usage()
{
	echo "usage: BRANCH=<branch> run-obmc-cppcheck.sh"
    echo "Example:"
    echo 'WORKSPACE=$(pwd) BRANCH=fw1120.00-1.70 openbmc-build-scripts/run-obmc-cppcheck.sh'
    echo ""
    echo "Notes:"
}
...
echo "usage:  run-obmc-cppcheck.sh $BRANCH"
...
[ -z "$SKIP_CODESCAN_PREPARE" ] && prepare_cppcheck

[ -z "$SKIP_CODESCAN_RUN" ] && run_cppcheck
